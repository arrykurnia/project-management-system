...
//daftar user
  router.get('/register', function(req, res, next) {
    res.render('register', { title: 'Register Account', message: req.flash('registerMessage') } );
  });
  ...
  router.post('/register', function(req, res, next) {
    ...
    db.query(`SELECT email FROM users WHERE email = '${req.body.email}'`, (err, data) => {
      ...
      if(data.rows.length > 0) {
        ...
        db.query(`INSERT INTO users( email, password, firstname, lastname, projectcolumns) VALUES('${req.body.email}','${passwordHash.generate(req.body.pass)}','${req.body.firstname}','${req.body.lastname}', '{}')`, (err, data) => {
          if(err) {
            console.error(err);
            req.flash('registerMessage', 'something wrong please call administrator');
            return res.redirect('/register')
